...
class LSTransformerDecoderFunc(Function):
    ...
    def backward(ctx, grad_output):
        ...
        return (grad_input, None, grad_enc_out, None, grad, None, None)
...
class LSTransformerDecoderLayer(nn.Module):
    ...
    def forward(
        self, decoder_states, decoder_padding_mask, encoder_out, encoder_padding_mask, cache, **kwargs
    ):
        """
        ...
        decoder_padding_mask = (
            (decoder_padding_mask * -1e8).type_as(decoder_states).contiguous()
        )
        ...
        if self.config.fp16:
            ...
            encoder_out = encoder_out.to(torch.half)
            encoder_padding_mask = encoder_padding_mask.to(torch.half)
            decoder_padding_mask = decoder_padding_mask.to(torch.half)
        ...
        if len(decoder_padding_mask.size()) == 1:
            assert decoder_states.size(0) == 1 and decoder_states.size(
                1
            ) == decoder_padding_mask.size(0)
        else:
            assert decoder_states.size(0) == decoder_padding_mask.size(
                0
            ) and decoder_states.size(1) == decoder_padding_mask.size(1)
