...
std::vector<torch::Tensor> transformer_decoder_layer_fw(
    int layer_id, const torch::Tensor &dec_input, const torch::Tensor &dec_mask,
  ...
  const T *dec_mask_ptr = (const T *)dec_mask.data_ptr();
  ...
  layer->SetTrainingMode(training_mode);
  layer->Forward(dec_input_ptr, dec_mask_ptr, enc_output_ptr, enc_mask_ptr, dec_output_ptr,
                 cache_ptr);
