...
/*------forward------*/
template <typename T>
void TransformerDecoderLayer<T>::self_attn_layer_fw(const T *input_ptr, const T *input_mask_ptr,
  ...
  _attn_scores.Forward(batch_heads, _soft_out_ptr, k_tf_ptr, q_tf_ptr,
                       _cublasHandle);
  // Softmax + Mask
  _softmax.Forward(_soft_out_ptr, input_mask_ptr, batch_size, from_len, to_len,
                   _stream);
...
void TransformerDecoderLayer<T>::Forward(const T *dec_input_ptr,
  ...
  self_attn_layer_fw(dec_input_ptr, dec_mask_ptr, encdec_attn_inp_ptr, buffer, cache);
